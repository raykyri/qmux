...
Enterprise-grade research engine for Claude Code. Produces citation-backed reports with source credibility scoring and automated validation.
...
No additional dependencies required. Searches use Claude Code's built-in WebSearch and WebFetch.
...
| Tool | Priority | Setup |
|------|----------|-------|
| WebSearch | **Primary** — all searches go here first | None (built-in) |
| WebFetch | Deep-dive extraction from specific URLs | None (built-in) |
| Exa MCP | Optional — semantic/neural search | MCP config |
...
| 2.3 | 2026-03-19 | Contract harmonization, dynamic year detection, disk-persisted citations, validation loops |
