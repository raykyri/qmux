...
fn shell_agent_functions(cli: &str, shell_commands: &[ShellCommandIntegration]) -> String {
    shell_commands
        ...
        .map(|command| {
            ...
            // ctrl-c, /exit, or a crash), covering the case where a stale agent status
            // would otherwise stick to the tab forever.
...
fn start_reader_thread(
    ...
    thread::spawn(move || {
        ...
        // retire that pending work with no way back. Capture a close snapshot first so it
